...
from cellpose.core import assign_device, run_net, run_3D
# XXX: Use Cellpose 3 transforms
...
class CellposeModel():
    ...
    def __init__(self, gpu=False, pretrained_model=False, model_type=None,
        ...
        self.gpu = device_gpu
        if not self.gpu:
            # XXX: Cellpose 4 disables support for mkldnn
            self.mkldnn = False
    ...
    def eval(self, x, batch_size=8, resample=True, channels=None, channel_axis=None,
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            # reshape image
            # XXX: convert image rewritten in CP4 so copied method from CP3
            x = convert_image(x, channels, channel_axis=channel_axis,
            ...
            if compute_masks:
                # XXX: niter must be integer
                niter0 = 200 if not resample else int(1 / rescale * 200)
